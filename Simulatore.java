...
public class Simulatore implements ActionListener,ItemListener{
    ...
    GraphicPanelStats Gstats_pane;
    GraphicPanelCellAutomata main_pane;
    Graphics Gmain_pane;

    JTextField field1,field2,field3,field4,FieldEntropy;//fields de pane Stats
...
//https://docs.oracle.com/javase/tutorial/displayCode.html?code=https://docs.oracle.com/javase/tutorial/uiswing/examples/components/MenuLookDemoProject/src/components/MenuLookDemo.java
   /*Simulatore(){
  	...
  	public void actionPerformed(ActionEvent e){
        ...
        else if("Basico".equals(generador))
        {
          main_pane.setGenerador(9);
        }
  		}
        ...
        if("DIBUJAR".equals(e.getActionCommand()))
        {
  		...
  		else if("ClearStats".equals(e.getActionCommand()))
        {
        	Gstats_pane.getGraphics().clearRect(0,0,640,240);
        }
    ...
    public Container createContentPane_Options1_child4()
    {
    	...
    	JButton inicio = new JButton("DIBUJAR"),fin = new JButton("ClearStats");
  		inicio.addActionListener(this);
  		fin.addActionListener(this);

  		String[] generadores = {"26-1a","26-1b","26-2","26-3","26-42","Fishman & Moore1","Fishman & Moore2","Randu","Basico"};
    ...
    public Container createContentPane_StatsChild2(){
      JPanel pane = new JPanel();
      pane.setBackground(new Color(176,201,247));
      pane.setPreferredSize(new Dimension(160,240));
      FieldEntropy = new JTextField("0000");
      pane.add(FieldEntropy);

      return pane;
    }

    public Container createContentPane_StatsChild1(){
      Gstats_pane = new GraphicPanelStats();
      Gstats_pane.setBackground(Color.yellow);
      Gstats_pane.setPreferredSize(new Dimension(480,240));
      return Gstats_pane;
    }

    public Container createContentPane_StatsFather(Container panel1,Container panel2){
      JPanel panelFather = new JPanel(new BorderLayout());
      panelFather.add(panel1,BorderLayout.WEST);
      panelFather.add(panel2,BorderLayout.CENTER);
      return panelFather;
    }

    public Container createContentPane_MainFrame(){
      main_pane = new GraphicPanelCellAutomata();
    ...
    private static void createAndShowGUI() {
        ...
        frame_stats.setContentPane(sim.createContentPane_StatsFather
          (sim.createContentPane_StatsChild1(),sim.createContentPane_StatsChild2()));

        frame_stats.pack();
    	  frame_stats.setVisible(true);
        
        sim.main_pane.setPaneStats(sim.Gstats_pane);
        sim.main_pane.setFieldEntropy(sim.FieldEntropy);
    }
...
class GraphicPanelCellAutomata extends JPanel{
  ...
  JTextField textFieldEntropy;
  GraphicPanelStats paneStats;

	public GraphicPanelCellAutomata(){
  ...
  public void setFieldEntropy(JTextField txtEntropy){ textFieldEntropy = txtEntropy;}
  public void setPaneStats(GraphicPanelStats statsPanel){paneStats = statsPanel;}
	...
	public void paintComponent(Graphics g){
		...
		if(veces_ > 1)
	    {
          ...
          int HammingAnterior = 0;
          paneStats.getGraphics().clearRect(0,0,630,240);
       		for(int y=0;y<480;++y)
       		{
            ...
            automata.nextGen();
            paneStats.paintHamming(paneStats.getGraphics(),automata,HammingAnterior,Ncelulas);
            HammingAnterior = automata.distanciaHamming();
            paneStats.paintEntropiaEspacial(paneStats.getGraphics(),automata);
            textFieldEntropy.setText(Double.toString(automata.entropia_temporalCell7()));
            //llamamos aquí al paintComponent del panel estadístico 
       		}
...
class GraphicPanelStats extends JPanel{
  private static final long serialVersionUID = 1L;//REVISAR
  double entropiaSiguiente_,entropiaAnterior_;

  GraphicPanelStats(){
    entropiaSiguiente_ = 0;
    entropiaAnterior_ = 0;
  }
  ...
  void paintHamming(Graphics gStats,ca1DSim cellAutomata,int anterior,int Ncelulas)
  {
    gStats.setColor(Color.red);
    float siguiente = (float)cellAutomata.distanciaHamming()/(float)Ncelulas;
    float anteriorPonder = (float)anterior/(float)Ncelulas;
    gStats.drawLine(cellAutomata.Ngeneraciones,getHeight()-(int)(anteriorPonder*getHeight()),cellAutomata.Ngeneraciones+1,getHeight()-(int)(siguiente*getHeight()));

  }
  void paintEntropiaEspacial(Graphics gStats,ca1DSim cellAutomata)
  {
    //super.paintComponent(gStats);
    gStats.setColor(Color.black);

    entropiaSiguiente_ = cellAutomata.entropia_espacial();    
    //gStats.fillRect((int)(cellAutomata.Ngeneraciones),(int)(entropiaSiguiente_*getHeight()),2,2);
    //System.out.println(cellAutomata.Ngeneraciones);
    gStats.drawLine(cellAutomata.Ngeneraciones,getHeight()-(int)(entropiaAnterior_*getHeight()),cellAutomata.Ngeneraciones+1,getHeight()-(int)(entropiaSiguiente_*getHeight()));
    entropiaAnterior_ = entropiaSiguiente_;
  }
}
